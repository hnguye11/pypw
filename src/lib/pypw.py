...
class SimAuto:
	...
	def Connect(self):
		...
		self.saco = cc.CreateObject("pwrworld.SimulatorAuto", dynamic=True) # create SimAuto COM Object
	...
	def TSRunAndPause(self, CtgName, StopTime, IsReset=False):
		...
		Script = "TSRunUntilSpecifiedTime(%s, [%s, %s, %s, %s, %s]);"% \
			(CtgName, StopTime, StepSize, StepsInCycles, ResetStartTime, NumberOfTimeStepsToDo)
